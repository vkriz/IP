...
from pj import *
##pytonov print, da ljepše ispišemo
import pprint


class SQL(enum.Enum):
    ##klasa znaci da može mijenjat vrijendnost
    class IME(Token): pass
    class BROJ(Token): pass
    ## tu je vrijednost uvijel select, from itd.
...
def sql_lex(kôd):
    ...
    for znak in iter(lex.čitaj, ''):
        ...
        ## - je znak da je pocetak komentara
        elif znak == '-':
            lex.pročitaj('-')
            lex.pročitaj_do('\n')
            lex.zanemari()##prociamo kometar i zanemarimo ga
        elif znak.isalpha():##ako si nasao znak
            lex.zvijezda(str.isalnum) ##proèitaj o ili vise(slovo ili znamenka)
            ##geldaj da li je neki od select, from itd. ako nije onda je sql.ime
            yield lex.literal(SQL.IME, case=False) ##false-case sensitive
        else: yield lex.literal(SQL)

print(*sql_lex('''\
CREATE bla bla --- 563sdflk
*, FROM from ) (
'''))
